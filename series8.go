...
func texture(u, v, t float64) float64 {
	...
	return spow(sin(u + strength(1+3*t)*subtexture2(u, v, t)) *
			...
			sin(u + v + strength(4+11*t)*subtexture2(u, v, t)), .5)/2+.5
}
...
func (s *SLR2) invisible(point geom.Vec) bool {
	...
	if projectedPoint.X < -projectedPoint.Z*1.19*1.1 || projectedPoint.X > projectedPoint.Z*1.19*1.1 {
		return true
	}
	if projectedPoint.Y < -projectedPoint.Z*1.19*1.1 || projectedPoint.Y > projectedPoint.Z*1.19*1.1 {
		return true
	}
	return false
...
func renderSurfaces(frameNumber int, pixels int, maxSubdivisions int, dt float64, desiredTriangles int) {
	...
	focusPoint := circle(t).Scaled(.0242)
	cameraLoc := geom.Vec{0, 0, 0}
	...
	if desiredTriangles > 0 {
		...
		fmt.Printf("minDistance: %v, maxDistance: %v, distance: %v\n", minDistance, maxDistance, distance)
	...
	for vIndex := 0; vIndex < nV; vIndex++ {
		for uIndex := 0; uIndex < nU; uIndex++ {
			...
			blendValue := 1 - float32(texture(index2radians(float64(uIndex), nU), index2radians(float64(vIndex), nV), t))
...
<scene version="2.0.0">
    <sensor type="thinlens" id="Camera-camera">
        ...
        <float name="aperture_radius" value=".00005"/>
        <float name="fov" value="50"/>
        <transform name="to_world">
            <lookat origin="{{ .LookAt.X }}, {{ .LookAt.Y }}, {{ .LookAt.Z }}" target="{{ .Camera.X }}, {{ .Camera.Y }}, {{ .Camera.Z }}" up="0, 1, 0"/>
        </transform>
...
func main() {
	...
	maxFrames := flag.Int("maxframes", 20, "Max frames")
